...
FROM
  employees
`
const results = pg<model, typeof query>(query);
...
const results2 = pg<model, typeof partialColumnsQuery>(partialColumnsQuery) // returned type contains only selected columns
...
const result3 = pg<model, typeof nonExistingColumnQuery>(nonExistingColumnQuery); // Returned type of result3 - TequelParseError<"Query requested column address which does not exist in requested tables">
...
const result4 = pg<model, typeof badTableName>(badTableName);